<template>
            ...
            <el-table stripe :data="scope.row.sr_list" style="width: 100%">
              ...
              <el-table-column label="操作" align="center"
                ><template #default="scope">
                  <el-button
                    type="warning"
                    v-on:click="sr_edit(scope.row.id)"
                    v-show="isdeveloper(scope.row.developer)"
                    size="small"
                    >编辑</el-button
                  >
                </template></el-table-column
  ...
  <ChangeDialog
    :dialogVisible="changedialog_visible"
    :_srid="srid"
    :_cancel="change_cancel"
    :_reload="reload"
  />
</template>
...
import ChangeDialog from '@/components/develop/ChangeDialog.vue'
import { getMyIr } from '@/utils/com_develop'
...
export default {
  ...
  methods: {
    ...
    sr_edit(srid) {
      this.srid = srid
      if (this.changedialog_visible === false) this.changedialog_visible = true
      else this.changedialog_visible = false
    },
    change_cancel() {
      this.changedialog_visible = false
      this.srid = 0
    },
    isdeveloper(name) {
    ...
    filter(tempir) {
      // remove same ir in the ir list
      this.ir = []
      const num = []
      for (let i = 0; i < tempir.length; i++) {
        const id = tempir[i].id
        const innum = num.some(function (val) {
          return val === id
        })
        if (!innum) {
          num.push(id)
          this.ir.push(tempir[i])
        }
      }
    }
  },
